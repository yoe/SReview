...
ok($val =~ /^Warning: could not find configuration file config, falling back to defaults at t\/1-config\.t line \d+\.$/, 'loading nonexisting config produces a warning but succeeds');
